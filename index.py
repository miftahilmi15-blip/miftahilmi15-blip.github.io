...
<head>
    ...
    <style>
        :root { --primary: #1f6f51; --grad: linear-gradient(135deg, #1f6f51, #2ecc71); --bg: #f8faf9; --shadow: 0 10px 30px rgba(31, 111, 81, .12); }
        * { box-sizing: border-box; font-family: 'Poppins', sans-serif; -webkit-tap-highlight-color: transparent; }
        ...
        .menu-btn { position: fixed; top: 22px; right: 22px; z-index: 2100; width: 48px; height: 48px; border-radius: 14px; background: #fff; color: var(--primary); display: none; align-items: center; justify-content: center; box-shadow: var(--shadow); cursor: pointer; }
        .sidebar { position: fixed; inset: 0; background: rgba(255, 255, 255, .98); z-index: 2000; transform: translateX(100%); transition: .4s cubic-bezier(0.4, 0, 0.2, 1); display: flex; flex-direction: column; align-items: center; justify-content: center; }
        ...
        .nav a { padding: 15px; border-radius: 18px; text-decoration: none; background: #fff; color: #333; font-weight: 600; display: flex; gap: 10px; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(0, 0, 0, .04); transition: .2s; }
        .card { background: #fff; padding: 35px 30px; border-radius: 35px; box-shadow: var(--shadow); }
        input { width: 100%; padding: 15px; border-radius: 14px; border: 1px solid #eee; margin-bottom: 14px; outline: none; font-size: 14px; }
        .btn { width: 100%; padding: 16px; border: none; border-radius: 16px; background: var(--grad); color: #fff; font-weight: 700; cursor: pointer; }
        .btn-google { margin-top: 20px; border: 1.5px solid #eee; background: #fff; display: flex; gap: 12px; align-items: center; justify-content: center; font-weight: 700; border-radius: 16px; padding: 14px; cursor: pointer; }
        .btn-google img { width: 22px; }
        .streak { background: var(--grad); border-radius: 24px; padding: 20px; color: #fff; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 15px 35px rgba(31, 111, 81, .25); }
        .fire { font-size: 38px; }
        .fire.off { filter: grayscale(1) opacity(.4); transform: scale(.85); }
        .mah { background: #fff; border-radius: 50px; padding: 14px 18px; margin: 22px 0; box-shadow: var(--shadow); display: flex; align-items: center; overflow: hidden; }
        .label { background: #d4b96a; color: #fff; font-size: 10px; padding: 5px 12px; border-radius: 50px; margin-right: 12px; font-weight: 800; white-space: nowrap; }
        .menu { display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; }
        .item { background: #fff; border-radius: 24px; padding: 20px 10px; text-align: center; box-shadow: var(--shadow); cursor: pointer; transition: .2s; }
        ...
        #ai-btn { position: fixed; bottom: 20px; right: 20px; z-index: 1200; width: 60px; height: 60px; border-radius: 50%; background: #15803d; color: #fff; display: none; align-items: center; justify-content: center; font-size: 24px; cursor: pointer; box-shadow: 0 5px 15px rgba(0,0,0,0.3); }
...
<body>
    ...
    <div id="ai-box">
        ...
        <div id="chat" class="chat-content"><div class="msg-ai">Assalamu’alaikum 😊 Ada yang bisa dibantu?</div></div>
    ...
    <script>
        const firebaseConfig = { apiKey: "AIzaSyCFVjE7ey5g__Iv4vUOZftB7g8GtSTSsEo", authDomain: "e-santri-b62be.firebaseapp.com", projectId: "e-santri-b62be" };
        firebase.initializeApp(firebaseConfig);
        const auth = firebase.auth();
        const menuBtn = document.getElementById('menuBtn'), sidebar = document.getElementById('sidebar'), closeBtn = document.getElementById('closeBtn'), aiBox = document.getElementById('ai-box'), aiBtn = document.getElementById('ai-btn');
