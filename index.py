...
# Gunakan API Key yang valid
genai.configure(api_key="AIzaSyCZmCTKtlYKcte4ytLmqhQbvZy7O3k5Ar4")

# Inisialisasi model dengan penanganan error agar tidak 404
try:
    model = genai.GenerativeModel('gemini-1.5-flash')
except:
    model = genai.GenerativeModel('gemini-pro')

# --- KODE HTML (GABUNGAN DASHBOARD + AI) ---
HTML_CODE = """
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .chat-input input { margin-bottom: 0 !important; border: none !important; font-size: 13px; }
    </style>
</head>
<body>
    ...
    <script>
        ...
        async function sendAI() {
            ...
            chat.innerHTML += `<div class="msg msg-ai" id="${loadId}">...</div>`;
...
def proses():
    try:
        ...
        # Jalankan AI
        response = model.generate_content(f"Berperanlah sebagai asisten santri yang bijak dan ramah. Jawab pertanyaan ini: {pertanyaan}")
        
        return jsonify({"jawaban": response.text})
    except Exception as e:
        return jsonify({"jawaban": f"Sistem sedang sibuk. (Error: {str(e)})"})
